...
allowlisted redistributable licence (`publicUsmle.media.kind=asset`). Unknown,
fair-use, private, or merely accessible media remains excluded. Text-only items
omit `media` (treated as `{ kind: 'none' }`). Design + pilot:
`docs/designs/2026-08-04-usmle-figure-lane.md`.
