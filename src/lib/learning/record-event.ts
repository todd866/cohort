...
export function computeConceptState(
  ...
  // Check if this is a probe (active test). Same set as ANSWER_EVENT_TYPES —
  ...
  if (existing?.lastExposureAt) {
    const daysSinceLastExposure = Math.max(
      0,
      (now.getTime() - existing.lastExposureAt.getTime()) / (24 * 60 * 60 * 1000),
    );
    recallProbability = applyDecay(
      recallProbability,
  ...
  let lastProbeAt: Date | null = existing?.lastProbeAt ?? null;
  ...
  // Compute event strength. A failed graded probe is evidence against recall,
  // regardless of which answer-event type supplied the grade. Negative quality
  // adjustments deepen the penalty; positive ones never turn a failure into a
  // learning gain.
  const baseStrength = EVENT_STRENGTH[eventType] ?? 0.1;
  ...
  if (
    ...
    qualityAdjustment = QUALITY_MULTIPLIERS[quality] ?? 0;
  }

  const qualitySuppliedOutcome = isCorrect === undefined && quality !== undefined;
