...
export function isRetryableReviewTransactionError(error: unknown): boolean {
  ...
  if (isPrismaErrorCode(error, 'P2034') || isPrismaErrorCode(error, 'P2002')) return true;

  // Prisma driver adapters can expose the native conflict before it is mapped
  // to P2034. Match the adapter's discriminant, never the error message.
  if (error === null || typeof error !== 'object' || !('name' in error)
    || error.name !== 'DriverAdapterError' || !('cause' in error)) return false;
