...
function isDecodeLeak(answer: string, before: string, after: string): boolean {
  ...
  // The answer must be expanded into its WORDS, not passed as one element:
  // initialsOf takes one letter per element, so a multi-word answer used to
  // contribute a single initial and could never match a multi-letter acronym.
  // "…is [haemolytic uraemic] syndrome (HUS)" read as H+S and slipped through.
