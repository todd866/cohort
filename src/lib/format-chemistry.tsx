 ...
 */
export interface FormatChemistryOptions {
  /** Rewrite "beta-blocker" as "β-blocker". Off on the review surface, where
   *  the ask was sub/superscripts and a symbol swap would change how thousands
   *  of cards read; on for the legacy callers that always had it. */
  greek?: boolean;
}

export function formatChemistry(text: string, options: FormatChemistryOptions = {}): ReactNode {
  if (!text) return text;

  // Greek substitution first (may change string length)
  const withGreek = options.greek === false ? text : substituteGreekLetters(text);
