...
type PrivateR2BucketEnvName =
  ...
  | 'CLOUDFLARE_R2_ANKI_ARTIFACTS_BUCKET_NAME';
...
function privateR2BucketName(name: PrivateR2BucketEnvName): string {
  ...
  const otherNames: PrivateR2BucketEnvName[] = [
    'CLOUDFLARE_R2_VIDEO_BUCKET_NAME',
    'CLOUDFLARE_R2_USER_DOCUMENTS_BUCKET_NAME',
    'CLOUDFLARE_R2_ANKI_ARTIFACTS_BUCKET_NAME',
  ].filter((candidate) => candidate !== name) as PrivateR2BucketEnvName[];
  for (const otherName of otherNames) {
    ...
    if (
      ...
      if (
        (name === 'CLOUDFLARE_R2_VIDEO_BUCKET_NAME'
          && otherName === 'CLOUDFLARE_R2_USER_DOCUMENTS_BUCKET_NAME')
        || (name === 'CLOUDFLARE_R2_USER_DOCUMENTS_BUCKET_NAME'
          && otherName === 'CLOUDFLARE_R2_VIDEO_BUCKET_NAME')
      ) {
        throw new Error(
          'CLOUDFLARE_R2_VIDEO_BUCKET_NAME and CLOUDFLARE_R2_USER_DOCUMENTS_BUCKET_NAME must use separate private buckets',
        );
      }
      throw new Error(`${name} and ${otherName} must use separate private buckets`);
    }
  }
...
export const config = {
  ...
  ankiArtifactsR2BucketName: () => privateR2BucketName(
    'CLOUDFLARE_R2_ANKI_ARTIFACTS_BUCKET_NAME',
  ),
