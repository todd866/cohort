...
export interface BulkUpsertOptions {
  ...
  /**
   * Card-only defense for tightly scoped release seeds. The full canonical
   * seed intentionally revives reviewed MDX rows; a scoped seed can set false
   * so a missed preflight cannot resurrect a soft-deleted card.
   */
  reviveDeletedCards?: boolean;
}
...
export async function bulkUpsertQuestions(
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    const values = batch.map(q => {
      ...
      ${sqlEscape(q.practiceLocale ?? null)},
    ...
    const sql = `
      ...
      ON CONFLICT ("id") DO UPDATE SET
        ...
        "practiceLocale" = EXCLUDED."practiceLocale",
...
export async function bulkUpsertCards(
  ...
  cards: Array<{
    ...
    /** Exam views this card serves. Omitted → [], which the conflict clause
     *  reads as "source is silent" and preserves whatever is already there. */
    moduleNodes?: string[];
  ...
  const onProgress = options.onProgress;
  const deletedAtConflictValue = options.reviveDeletedCards === false
    ? '"Card"."deletedAt"'
    : 'NULL';
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    const values = batch.map(c => `(
      ...
      ${sqlEscape(c.clipId ?? null)},
      ${sqlEscape(c.clipRole ?? null)},
      ${sqlEscape(c.clipCaption ?? null)},
      ${sqlEscapeTextArray(c.moduleNodes ?? [])},
      ${sqlEscape(c.jurisdiction ?? null)},
      ${sqlEscape(c.practiceLocale ?? null)},
    ...
    const sql = `
      INSERT INTO "Card" (
        ...
        "crosslinks", "annotations", "abbreviations", "imageUrl", "imageCaption", "imageRole",
        "clipId", "clipRole", "clipCaption",
        "moduleNodes",
        "jurisdiction", "practiceLocale", "conceptId",
      ...
      ON CONFLICT ("stableId") DO UPDATE SET
        ...
        -- The GSSE and NSx views are composed ENTIRELY from this column, and the
        -- plate corpus has its assignments set outside the MDX. A plain
        -- EXCLUDED would blank all 17k of them on the next full seed, emptying
        -- both views. An empty incoming array means "this source is silent",
        -- not "clear it" — same reasoning as clusterId below.
        "moduleNodes" = CASE WHEN cardinality(EXCLUDED."moduleNodes") = 0 THEN "Card"."moduleNodes" ELSE EXCLUDED."moduleNodes" END,
        ...
        "variantType" = EXCLUDED."variantType",
        "deletedAt" = ${deletedAtConflictValue}
    `;
