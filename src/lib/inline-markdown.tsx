...
export function extractMarkdownTable(text: string): { table: ParsedTable; prose: string } | null {
  const normalized = normalizeInlineTables(decodeHtmlEntities(normalizeAngleBracketEscapes(text)));
  if (!hasMarkdownTable(normalized)) return null;
...
function renderWithTable(text: string, leaf: LeafRenderer): ReactNode {
  ...
  parts.push(<MarkdownTable key={key++} table={parsed} leafRenderer={leaf} />);
