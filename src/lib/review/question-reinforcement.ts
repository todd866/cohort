...
export function buildPrimaryCardDraft(
  ...
  return {
    ...
    imageRole: questionImageIsPromptWithLookup(
      question.imageRole,
      question.imageUrl,
...
export function computePrimaryCardUpdate(
  ...
  if ((existing.imageRole ?? null) !== (desired.imageRole ?? null)) updateData.imageRole = desired.imageRole;
  ...
  if (primaryCard) {
    ...
    if (softDeleted.length > 0) {
      await prisma.$executeRawUnsafe(
        `UPDATE "Card" SET "deletedAt" = NULL, "front" = $1, "back" = $2, "context" = $3, "topics" = $4, "difficulty" = $5, "imageUrl" = $6, "imageCaption" = $7, "imageRole" = $8, "rotation" = $9, "week" = $10 WHERE id = $11`,
      ...
      const newCard = await prisma.card.create({
        data: {
          ...
          imageRole: desiredPrimaryCard.imageRole,
        },
        select: { id: true },
      });
