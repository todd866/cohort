...
type RecordQuestionAttemptBackgroundInput = Omit<RecordQuestionAttemptInput, 'clientRequestId'> & {
  isCorrect: boolean;
  /** The learner declined to answer; the event is written NOT GRADED. */
  skipped?: boolean;
  /**
...
export type RecordQuestionAttemptOk = {
  ok: true;
  isCorrect: boolean;
  /** True when the learner declined to answer. isCorrect is false then, but it
   *  is not a wrong answer. Optional because a replayed receipt stored before
   *  this field existed has no value for it. */
  skipped?: boolean;
...
export async function recordQuestionAttemptFast(
  ...
  // Personal-deck ownership does not gate the public Step 1 release. `usmle-step1`
  // is registered as a personal deck, so this check rejected every anonymous
  // visitor on cohort.md — the read path served them a question and the write
  // path then answered 404, making the public product unusable by anyone but the
  // deck owner.
  //
  // This is not a loosening: reaching here with `isPublicUsmleDelivery` already
  // required the id to be in the checked-in open release with a matching serving
  // fingerprint (the 409 above), so the only rows it could ever have protected
  // are MIT/FOSS by construction. Private Step 1 content never takes this branch —
  // it has no public capability, and is failed closed above.
  if (!isPublicUsmleDelivery
    && !await userIdCanAccessRequestedRotations(userId, [rotation ?? ''])) {
    return { ok: false, status: 404, error: 'Question not found' };
  }
  ...
  // filter on selectedOption; the LearningEvent, whose isCorrect is nullable,
  // records a skip as NOT GRADED so every event reader that filters or
...
export async function recordQuestionAttemptBackground(
  ...
  try {
    ...
    //
    // Wrong answers AND skips: both mean the learner did not know, and
    // scaffolding is the response to not knowing. This is the one place a
    // skip is deliberately treated like a miss. Everywhere that MEASURES —
    // accuracy, readiness pressure, acute-failure exclusion, the rung
    // analysis — a skip is not graded (isCorrect null on the event, SKIP on
    // the response) and must not count as a failure.
    if (!isCorrect) {
