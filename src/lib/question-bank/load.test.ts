...
describe('loadQuestionBankFromDisk', () => {
  ...
  it('retains a validated prompt image role from disk', () => {
    ...
    try {
      fs.writeFileSync(path.join(promptDir, 'prompt.json'), JSON.stringify({
        ...makeQuestion('bank:toc:abdominal-prompt:v1', 'toc'),
        imageUrl: '/figures/restricted/prompt/toc/abdominal.png',
        imageCaption: 'A required abdominal examination finding.',
        imageRole: 'prompt',
      }));
      ...
      expect(result.errors).toEqual([]);
      expect(result.questions[0]).toMatchObject({ imageRole: 'prompt' });
    } finally {
      fs.rmSync(promptDir, { recursive: true, force: true });
    }
  });

  it('rejects prompt semantics without an image instead of silently downgrading', () => {
    const promptDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qbank-prompt-role-'));
    try {
      fs.writeFileSync(path.join(promptDir, 'prompt.json'), JSON.stringify({
        ...makeQuestion('bank:toc:broken-prompt:v1', 'toc'),
        imageRole: 'prompt',
      }));
      const result = loadQuestionBankFromDisk({
        bankDir: promptDir,
      ...
      expect(result.questions).toEqual([]);
      expect(result.errors.join('\n')).toMatch(/imageRole=prompt requires imageUrl/);
    } finally {
      fs.rmSync(promptDir, { recursive: true, force: true });
    }
  });
