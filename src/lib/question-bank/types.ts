...
export type RotationId =
  ...
  | 'year3-common'
  // Open GAMSAT reasoning corpus (cohort.md/gamsat)
  | 'gamsat';
...
export interface CuratedQuestion {
  ...
  topics: string[];
  /**
   * Shared slug binding the easy/medium/hard rungs of one concept ladder.
   * Mirrored as topic `ladder:<ladderId>` so topic-based concept matching
   * co-locates the rungs (see scripts/usmle/ladder-link.ts).
   */
