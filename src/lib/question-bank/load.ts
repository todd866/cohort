...
const CuratedQuestionSchema = z.object({
  ...
  ladderId: z.string().trim().min(1).nullable().optional(),
  ...
  imageRole: z.literal('prompt').nullable().optional(),
  clipSlug: z.string().trim().min(1).nullable().optional(),
  clipRole: z.literal('prompt').nullable().optional(),
...
export const ROTATION_DIRS = new Set([
  ...
  'clinical-skills', 'mmca',
  // USMLE — 'usmle-step1' is the owner-gated personal Step 1 material;
  // 'usmle-step1-open' is the MIT/CC-BY corpus cohort.md serves to visitors.
  'usmle-step1', 'usmle-step1-open',
  ...
  'mnd',
  // Personal Talley & O'Connor clinical-examination deck.
  'toc',
