...
export interface Reporter {
  isAdmin: boolean;
  /**
   * A known-good human on the trusted-reporter allowlist (see
   * `flags/trusted-reporters.ts`). Treated like an admin for auto-approval —
   * the classmates who actually study here file the best content reports, and
   * quarantining them means those reports never reach triage at all.
   */
 ...
 * markers, and a rejected admin note falls back to quarantine. An allowlisted trusted
 * reporter takes the same path for the same reason. Everyone else stays
 * quarantined, awaiting a reviewer or a screener.
 */
export function trustDecisionForReport(
  ...
  if (!reporter.isAdmin && !reporter.isTrustedReporter) return { state: 'quarantined' };
  ...
  return {
    ...
    // Distinguishable in the audit trail: an allowlisted classmate is not the
    // repo owner, and a reviewer looking at /x/flags should be able to tell.
    trustReviewedBy: reporter.isAdmin ? 'auto:admin' : 'auto:trusted-reporter',
