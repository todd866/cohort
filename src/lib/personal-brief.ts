...
export interface Deadline {
  ...
  kind: string;
  /**
   * Explicit workflow state. Past legacy rows without this field remain
   * historical instead of becoming surprise overdue work.
   */
...
export interface DayView {
  ...
  /** Active deadlines, including explicitly unfinished overdue rows. */
...
export interface ResolveOptions {
  ...
  /** Days ahead of an ordinary deadline at which it becomes `due-soon`. */
  deadlineDueSoonWindowDays?: number;
}
...
/**
 * Resolve an ordinary deadline without turning old, untyped data into noisy
 * overdue work. A past row is actionable only when its lifecycle was authored
 * explicitly; submitted/completed rows disappear regardless of date.
 */
export function deadlineUrgency(
  ...
  if (deadline.lifecycle === 'submitted' || deadline.lifecycle === 'completed') return 'done';
  const delta = daysBetween(date, deadline.dueDate);
  if (delta < 0) return deadline.lifecycle ? 'overdue' : 'historical';
  if (delta <= dueSoonWindowDays) return 'due-soon';
  return 'upcoming';
}
...
export function resolveDay(
  ...
  const {
    prioritiseActions = false,
    dueSoonWindowDays = DEFAULT_DUE_SOON_WINDOW,
    deadlineDueSoonWindowDays = DEFAULT_DUE_SOON_WINDOW,
  } = options;
  ...
  const upcomingDeadlines: DeadlineView[] = brief.deadlines
    .map((deadline) => ({
      ...deadline,
      urgency: deadlineUrgency(deadline, date, deadlineDueSoonWindowDays),
      daysUntilDue: daysBetween(date, deadline.dueDate),
    }))
    .filter((deadline) => deadline.urgency !== 'done' && deadline.urgency !== 'historical')
...
export function resolveDayEvents(
  ...
  for (const d of brief.deadlines) {
    ...
    if (d.dueDate === date && urgency !== 'done' && urgency !== 'historical') {
      out.push(deadlineEvent(brief, d));
    }
  }
...
export function resolveUpcomingEvents(
  ...
  for (const d of brief.deadlines) {
    const urgency = deadlineUrgency(d, date);
    if (urgency !== 'done' && urgency !== 'historical') out.push(deadlineEvent(brief, d));
  }
