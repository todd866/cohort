...
import { withholdsFromServing } from './withholding-severity';
...
export async function getOpenIssueExclusions(): Promise<{
  ...
  try {
    ...
    for (const issue of issues) {
      // Only a correctness defect is worth withholding the item over. Cosmetic
      // and pedagogical reports (construction nits, needs-image, too-easy)
      // leave the content served — see withholding-severity.ts.
      if (!withholdsFromServing(issue.issueType)) continue;
