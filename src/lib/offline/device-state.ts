...
import { clearPack, rekeyPackOwner, storedPackOwnerKey } from './pack';
import { clearOfflineReviewProgress } from './progress';
import { clearImageChoiceHistory } from '@/lib/review/image-choices';
import {
...
export function bindOfflineOwner(ownerKey: string): OwnerLease {
  ...
  if (switched) {
    ...
    const deviceGuestSignIn = Boolean(
      previousOwner
      && !previousOwner.verified
      && previousOwner.ownerKey.startsWith(DEVICE_GUEST_OWNER_PREFIX),
    );
    if (deviceGuestSignIn) {
      // Keep the warm pack under the authenticated key so a remount can paint
      // immediately. Guest review rows move with the outbox claim below.
      rekeyPackOwner(previousOwner!.ownerKey, lease.ownerKey);
