...
/**
 * After a claim attempt, drop the HttpOnly guest cookie whenever retrying
 * cannot recover progress for this cookie. Keeping a stale cookie forces the
 * authenticated app to re-POST claim forever (and contend for Neon locks).
 */
export function shouldClearGuestCookieForClaimResult(
  result: GuestProgressClaimResult,
): boolean {
  if (result.status === 'claimed' || result.status === 'nothing-to-claim') {
    return true;
  }
  if (result.status !== 'not-eligible') return false;
  return (
    result.reason === 'guest-missing'
    || result.reason === 'guest-not-anonymous'
...
export async function claimGuestProgressRecords(
  ...
  // A guest can now pick their rotation before signing up. Carry that choice
  // across, or the chooser is answered twice and asking before the first card
  // buys nothing. Strictly fill-if-empty: a returning user who happened to
  // study anonymously keeps the enrolment already on their account.
  const accountHasEnrolment = (authenticatedUser.activeModules ?? []).length > 0;
  const guestEnrolment = accountHasEnrolment
    ? {}
    : {
        ...((guest.activeModules ?? []).length > 0
          ? { activeModules: guest.activeModules }
          : {}),
        ...(authenticatedUser.track === null && guest.track !== null
          ? { track: guest.track }
          : {}),
      };
...
export async function claimGuestProgressAfterSignIn(
  ...
  if (!guestUserId) {
    return finishClaim({
      authenticatedUserId,
      guestUserId: null,
      result: { status: 'not-eligible', reason: 'cookie-missing' },
      startedAt,
    });
  }
  ...
  if (guestUserId === authenticatedUserId) {
    return finishClaim({
      authenticatedUserId,
      guestUserId,
      result: { status: 'not-eligible', reason: 'same-user' },
      startedAt,
    });
  }

  // Cheap eligibility pre-check: a deleted or non-anonymous cookie must not
  // open a serializable FOR UPDATE against the authenticated user. That path
  // was starving review writes and leaving a sticky retry loop when Neon was
  // slow and the client aborted before Set-Cookie cleared.
  let guestPreview: {
    id: string;
    email: string | null;
  ...
  try {
    guestPreview = await prisma.user.findUnique({
      where: { id: guestUserId },
      select: {
        id: true,
        email: true,
  ...
  if (!guestPreview) {
    return finishClaim({
      authenticatedUserId,
      guestUserId,
      result: { status: 'not-eligible', reason: 'guest-missing' },
      startedAt,
    });
  }
  if (guestPreview.email !== null || guestPreview.accounts.length > 0) {
    return finishClaim({
      authenticatedUserId,
      guestUserId,
      result: { status: 'not-eligible', reason: 'guest-not-anonymous' },
      startedAt,
    });
  }
...
async function finishClaim(args: {
  authenticatedUserId: string;
  guestUserId: string | null;
  result: GuestProgressClaimResult;
  startedAt: number;
}): Promise<GuestProgressClaimResult> {
  const { authenticatedUserId, guestUserId, result, startedAt } = args;
  const clearedCookie = shouldClearGuestCookieForClaimResult(result);
  if (clearedCookie) {
    try {
      await clearGuestUserCookie();
    } catch (error) {
      // Claimed/nothing-to-claim already deleted the guest identity. Terminal
      // not-eligible means this cookie can never import progress. Either way a
      // sticky cookie is worse than a clear failure — log and continue.
  ...
  // cookie-missing is the steady-state for signed-in browsers with no guest
  // identity — logging it would drown real claim outcomes.
  if (!(result.status === 'not-eligible' && result.reason === 'cookie-missing')) {
    ...
    logger.info('guest-progress-claim', {
      authenticatedUserId,
      guestUserId,
