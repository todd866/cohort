...
export function deriveCardDifficulty(card: {
  ...
  // C4/C5 added 2026-09-14 alongside the ladder extension. They sit above C3
  // but the curve flattens deliberately: a board vignette is harder than a
  // complex recall card, and not by as much again as C1 is below C3 — the
  // difficulty of an exam item comes from the elimination it demands, which
  // this text-derived score cannot see. Leaving 4 and 5 to fall through the
  // `?? 0.5` default would have scored them EASIER than a C3.
  const baseByComplexity: Record<CardComplexity, number> = {
    1: 0.22, 2: 0.5, 3: 0.82, 4: 0.9, 5: 0.95,
  };
