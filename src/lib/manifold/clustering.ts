...
export async function getClusterMastery(
  ...
  options: {
    ...
    now?: Date;
    /**
     * Companion rotations whose clusters this session may also draw from.
     *
     * A view rotation (`neurosurg`) owns no cards: every card it serves lives
     * in a host corpus and declares the view in `moduleNodes`. Its clusters are
     * therefore stamped with the HOST's rotation, so asking for
     * `rotations has 'neurosurg'` finds nothing and the session comes back
     * empty. Widening here is coarse eligibility only — the caller still has to
     * scope the cards themselves with `sessionCandidateItemWhere`.
     */
    additionalRotations?: readonly string[];
  } = {},
): Promise<ClusterMastery[]> {
  const { maxAgeMs = 60 * 60 * 1000 } = options; // Default 1 hour cache
  const rotationScope = [rotation, ...(options.additionalRotations ?? [])];

  // Get all clusters for this rotation (plus any companion rotations).
  const clusters = await prisma.cluster.findMany({
    where: {
      rotations: { hasSome: rotationScope },
    },
    select: {
      id: true,
      name: true,
  ...
  if (missingOrStale.length > 0) {
    ...
    await refreshClusterMastery(userId, rotationScope, missingOrStale, options.now);
...
async function refreshClusterMastery(
  userId: string,
  rotationScope: readonly string[],
  clusterIds: string[],
  frozenNow?: Date,
): Promise<void> {
  const now = frozenNow instanceof Date && Number.isFinite(frozenNow.getTime())
    ? frozenNow
    : new Date();
  ...
  const progressData = await prisma.card.findMany({
    where: {
      // Scope, not equality: a view rotation's cards live in its host corpus,
      // so pinning `rotation` here would report every cluster as 0-reviewed.
      rotation: { in: [...rotationScope] },
      deletedAt: null,
      shelvedAt: null,
      clusterId: { in: clusterIds },
    },
