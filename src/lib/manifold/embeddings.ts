...
/**
 * Resolve a web-root-relative image path without allowing filesystem paths,
 * URL schemes, encoded traversal, backslashes, or escapes from `public/`.
 * This is defense in depth: broad content embedding callers must still require
 * durable positive provider-consent provenance before supplying any image.
 */
export function resolveProviderImagePath(
  ...
  const relativeInput = raw.startsWith('/') ? raw.slice(1) : raw;
  const segments = relativeInput.split('/');
  if (
    segments.length === 0
...
function imagePartFromPublicPath(imageUrl: string | null | undefined): EmbeddingPart | null {
  ...
  const fullPath = resolveProviderImagePath(imageUrl, publicRoot);
  if (!fullPath) return null;

  const mimeType = SUPPORTED_IMAGE_MIME[path.extname(fullPath).toLowerCase()];
  if (!mimeType) return null;

  const canonicalFile = resolveReadableProviderImagePath(imageUrl, publicRoot);
  if (!canonicalFile) return null;

  return {
    inlineData: {
      data: fs.readFileSync(canonicalFile).toString('base64'),
      mimeType,
    },
  };
}
...
/**
 * Resolve an existing regular file and reject symlinks that escape public/.
 * Exported so the filesystem containment invariant can be tested directly.
 */
export function resolveReadableProviderImagePath(
  imageUrl: string | null | undefined,
  publicRoot = path.resolve(process.cwd(), 'public'),
): string | null {
  const fullPath = resolveProviderImagePath(imageUrl, publicRoot);
  ...
  // A symlink inside public/ must not be able to point at a private local file.
  let canonicalRoot: string;
  let canonicalFile: string;
  try {
    canonicalRoot = fs.realpathSync(publicRoot);
    canonicalFile = fs.realpathSync(fullPath);
  } catch {
    return null;
  }
  if (!isContainedPath(canonicalRoot, canonicalFile)) return null;
