...
export function updateMasteryState(
  ...
  // Preserve the original graduation timestamp while a mastered/retired card
  // remains mastered. Returning null is not an inert "no change" marker: the
  // live Prisma writer persists it and would erase the timestamp.
  let masteredAt = currentState.masteredAt ?? null;
  ...
  if (currentState.status === 'mastered' && !isCorrect) {
    ...
    newStatus = 'mastered';
    masteredAt = new Date(review.reviewedAt ?? Date.now());
