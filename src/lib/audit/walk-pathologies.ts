import type { Session, SessionMetrics, Pathology } from './walk-types';
import {
  collapseSiblingsForMetrics,
  countImmediateScaffoldEvaluableMisses,
} from './walk-metrics';
...
export function detectPathologies(
  ...
  const evaluableMissCount = countImmediateScaffoldEvaluableMisses(
    session.trajectory,
    session.responses,
  );
  if (
