...
/**
 * Map an MCQ's authored/effective difficulty onto the same three-rung scale
 * used by cards. Unknown values stay unknown rather than silently becoming
 * `standard`, because this function feeds durable telemetry and audit math.
 */
export function tierFromQuestionDifficulty(
  difficulty: unknown,
): 'scaffolding' | 'standard' | 'stretch' | null {
  if (typeof difficulty !== 'string') return null;
  const normalized = difficulty.trim().toLowerCase();
  if (normalized === 'easy') return 'scaffolding';
  if (normalized === 'medium') return 'standard';
  if (normalized === 'hard') return 'stretch';
  return null;
}
