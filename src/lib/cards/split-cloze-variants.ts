...
export function splitClozeVariants(
  ...
  //
  // A hand-authored sibling (variantGroup= in MDX, per the variant-grouping
  // rule) also arrives with variantGroupId set but carries NO stableId of its
  // own, so it still needs one assigned here. Already-split cards keep their
  // exact identity, which callers rely on.
  if (card.variantGroupId != null) {
    const identified = card.stableId != null
      ? card
      : { ...card, stableId: baseStableId };
    return { cards: [identified], legacyAnchorStableIds: [], warnings: [] };
  }
