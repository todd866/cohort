import type { CardComplexity } from '@/lib/card-generator';
/**
...
/** Extract a JSX string-array attribute such as topics={["BLS", "CPR"]}. */
function extractStringArrayAttr(attrs: string, name: string): string[] {
...
function topicsFromHeadingText(raw: string): string[] {
  ...
  // A colon or a spaced dash separates a heading label from its detail. An
  // unspaced hyphen is part of the clinical term (for example
  // "Attention-Deficit/Hyperactivity Disorder" or "First-Episode
  // Psychosis") and must survive as reviewed topic metadata.
...
function extractComponentCards(
  ...
  config: {
    ...
    getComplexity: (back: string, front: string) => CardComplexity;
  ...
  while ((match = tagRegex.exec(content)) !== null) {
    ...
    const explicitTopics = extractStringArrayAttr(attrs, 'topics');
    const resolvedTopics = dedupeTopics([...headingTopics, ...explicitTopics]);
    ...
    const practiceLocaleRaw = extractStringAttr(attrs, 'practiceLocale');
    const practiceLocale =
      practiceLocaleRaw === 'au' || practiceLocaleRaw === 'us' ? practiceLocaleRaw : undefined;
    ...
    // Authored variant-group membership. The splitter derives this
    // mechanically for blank-splits of one sentence; this is the path for a
    // genuinely reworded sibling to join its parent's group, so the scheduler
    // can serve an unseen probe of the same fact instead of repeating one.
    const variantGroupId = extractStringAttr(attrs, 'variantGroup');
    ...
    // Operative video clip. Referenced by slug, not by id: MDX must be writable
    // before the media is cut, and the slug survives a re-cut. Resolved to a
    // VideoClip at seed time — an unresolvable slug is a seed-time error, not a
    // silently clip-less card.
    // Which exam views this card serves. The GSSE/NSx/anatomy split is declared
    // per-card in moduleNodes rather than by copying a card into three
    // rotations (see PERSONAL_DECKS) — NSx in particular is a rotation holding
    // no cards of its own, composed entirely from what other corpora declare
    // for it. The plate generator sets these directly; this is the path for
    // hand-authored MDX. Undefined when absent, never [], so a reseed cannot
    // wipe assignments made elsewhere.
    const modulesAttr = extractStringAttr(attrs, 'modules');
    const moduleNodes = modulesAttr
      ? modulesAttr.split(',').map((m) => m.trim()).filter(Boolean)
      : undefined;
    const clipSlug = extractStringAttr(attrs, 'clip') ?? extractStringAttr(attrs, 'clipSlug');
    const clipRole = extractStringAttr(attrs, 'clipRole');
    const clipCaption = extractStringAttr(attrs, 'clipCaption');
    ...
    // <KeyPoint> is the documented way to author a C1 teaching scaffold.
    //
    // THE CEILING IS 5, NOT 3, since 2026-09-14. A recall ladder that stops at
    // "complex fact" is not an exam ladder: a board-style vignette asks for a
    // MECHANISM after an unscored diagnostic step, and eliminates each
    // distractor on two independent axes. There was nowhere to put such an item
    // — this parser silently dropped `complexity={4}` back to the heuristic, so
    // authoring one was impossible rather than merely undocumented.
    // See docs/COMPLEXITY_LADDER.md for what each tier means.
    const complexityAttr = extractNumberAttr(attrs, 'complexity');
    const complexityOverride =
      complexityAttr !== undefined
        && Number.isInteger(complexityAttr)
        && complexityAttr >= 1
        && complexityAttr <= 5
        ? (complexityAttr as CardComplexity)
        : undefined;
...
export function extractKeyPointCards(
  ...
  return extractComponentCards(content, rotation, week, headings, {
    ...
    getComplexity: (back, front) => calculateComplexity('KeyPoint', back, front) as CardComplexity,
