...
import { resolveNeonConnectionTimeoutMs } from '@/lib/neon-connection-timeout';
import { isRepositoryScopedCardWhere } from '@/lib/cards/read-scope-registry.server';
...
// Bound only connection acquisition here; query execution keeps its existing
// semantics. The read-only Auth.js session lookup owns the single safe retry.
// A deployed request keeps the tight 3s bound; a local script gets more, since
// the Neon WebSocket handshake off Vercel's network is far slower than the
// database itself. See src/lib/neon-connection-timeout.ts.
export const NEON_CONNECTION_TIMEOUT_MS = resolveNeonConnectionTimeoutMs();
...
function extendPrismaClient(base: PrismaClient) {
  // ---------------------------------------------------------------------------
  // Card read boundary: fail closed to the shared catalog, auto-filter deleted
  // cards, and exclude answer-bearing QuestionReinforcement rows derived from
  // protected public Step 1 questions. Owner-private reads must carry an
  // verified where object built by the scoped Card repository.
  ...
  const withCardReadBoundaries = (
    ...
    const hasRepositoryScope = isRepositoryScopedCardWhere(input);
    const where: Prisma.CardWhereInput = { ...(input ?? {}) };
    if (!('deletedAt' in where)) where.deletedAt = null;
    if (!hasRepositoryScope) where.ownerUserId = null;
