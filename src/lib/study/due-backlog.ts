...
export async function fetchDueBacklogCards(
  ...
  // Fill the available backlog capacity from native rows first. Only a bounded
  // remainder may come from an authorized, target-matched source partition;
  // source rows can therefore never occupy the native query's database LIMIT.
  ...
  ].map((row) => ({ id: row.card.id, nextDueAt: row.nextDueAt }));
}
...
export async function appendDueBacklog(
  ...
  const dueCards = await fetchDueBacklogCards(prisma, {
    ...
    weekFilter,
    excludeIds: exclude,
    // Query only the actual open capacity. This lets the fetcher decide how
    // much source fill is needed after exhausting native backlog rows.
