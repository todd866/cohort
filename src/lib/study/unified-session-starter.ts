...
import { getOriginalImageAlternatives } from '@/lib/figures/original-image-alternatives';
import { imageAlternativeTeachingFingerprint } from '@/lib/figures/image-alternative-fingerprint';
import { loadCurrentSessionContent, sessionSourceKey, type CurrentSessionSource } from './current-session-content';
...
export async function tryStarterSession(ctx: SessionContext): Promise<NextResponse | null> {
  ...
  // Generated starter artifacts are rotation-wide. A topic deep link must
  // fall through to a lane that can prove every delivered item matches.
  ...
  // Generated starter bodies are not current-source evidence. Only mapped
  // items incur these selected-ID reads, and an unavailable source merely
  ...
  const starterItemsRaw = (await Promise.all(starterSession.items.map(async (item) => {
    ...
    try {
      resolved = await resolveImage(item.imageUrl ?? null, null, trustOverride);
    } catch (err) {
      logger.warn('starter: image resolve failed', { itemId: item.id, err });
    }
    const imageIsRequired = questionImageIsPrompt(item.imageRole, item.imageUrl);
    if (imageIsRequired && !resolved) {
      return null;
    }
    const alternativeSource = item.type === 'card' || item.type === 'question'
      ? alternativeSources.get(sessionSourceKey(item.type, item.id)) : undefined;
    const sourceMatchesStarter = alternativeSource
      && (item.type === 'card' || item.type === 'question')
      && (alternativeSource.imageUrl ?? null) === (item.imageUrl ?? null)
      && imageAlternativeTeachingFingerprint(alternativeSource)
        === imageAlternativeTeachingFingerprint({ ...item, type: item.type });
    return {
      ...item,
      ...
      imageAlternatives: sourceMatchesStarter
        ? await resolveImageAlternatives(alternativeSource, alternativeSource.imageUrl ?? null, null, trustOverride) : [],
      liked: false,
      flagged: false,
  ...
  const starterCardRows = starterItemsRaw.filter((item) => item.type === 'card');
  const [eligibleStarterCards, starterCardDue] = await Promise.all([
    ...
    filterCardsAtDueEgress(starterCardRows as UnifiedItem[], {
      userId: ctx.userId,
      rotation: ctx.rotation,
      path: 'starter',
      isGuest: ctx.isGuest,
  ...
  const eligibleStarterItemsRaw = starterItemsRaw.filter((item) => {
    ...
    if (item.type === 'card') {
      return eligibleStarterCardIds.has(item.id)
        && starterCardDue.eligibleCardIds.has(item.id);
    }
    return true;
  });
    ...
    try {
      const exposureEvents = starterItemsWithDecisions.map((item, index) => ({
        ...
        metadata: compactMetadata({
          ...
          srsDueGateVersion:
            item.type === 'card' ? starterCardDue.audit?.version : undefined,
          srsEligibilityCheckedAt:
            item.type === 'card'
              ? starterCardDue.audit?.checkedAt.toISOString()
              : undefined,
          srsProgressFound:
            item.type === 'card' && starterCardDue.audit
              ? starterCardDue.audit.nextDueAtByCardId.has(item.id)
              : undefined,
          srsNextDueAtAtOffer:
            item.type === 'card' && starterCardDue.audit
              ? starterCardDue.audit.nextDueAtByCardId.get(item.id)?.toISOString() ?? null
              : undefined,
        }),
      }));
  ...
  const starterTiming = [
    ...
    `starter;dur=${(tStarterEnd - ctx.tAuthEnd - ctx.tContentMapMs).toFixed(1)}`,
