...
export interface ManifoldExclusionState {
  ...
  /** Delivered/reviewed card ids inside the 24h repeat window only. */
  recentCardIds: Set<string>;
  recentTopicExposures: TopicExposureMap;
  enabledGroupTypes: string[];
  /**
   * Cards actually DELIVERED to this user recently, graded or not.
   *
   * The relearn lane deliberately bypasses `excludedCardIds` — re-showing a
   * just-failed card is its entire purpose — so it needs the raw delivery
   * history to enforce its own cap. Derived from rows already loaded here, so
   * exposing it costs no extra query. See `capRelearnCardsByDelivery`.
   */
  recentCardDeliveries: Array<{ cardId: string; deliveredAt: Date }>;
}
...
export function buildManifoldExclusionState(
  ...
  for (const event of options.recentExposureEvents) {
    if (event.sourceType === 'card' && event.timestamp >= options.cardRepeatCutoff) {
      ...
      recentCardIds.add(event.sourceId);
    }
  ...
  for (const decision of options.recentServeDecisions ?? []) {
    if (decision.itemType === 'card' && decision.decidedAt >= options.cardRepeatCutoff) {
      ...
      recentCardIds.add(decision.itemId);
    }
  ...
  return {
    ...
    enabledGroupTypes,
    recentCardDeliveries: (options.recentServeDecisions ?? [])
      .filter((decision) => decision.itemType === 'card')
      .map((decision) => ({ cardId: decision.itemId, deliveredAt: decision.decidedAt })),
  };
}
