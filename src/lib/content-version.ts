 ...
 * `{ in: ids }` filter spends one per id. Both bulk version writers passed the
 * entire pending list, so they worked fine until the corpus grew past the
 * ceiling — which happened on 2026-09-13 when a 1,793-card neuroanatomy import
 * pushed a groupBy over it and failed the whole seed with P2029.
 *
 * 5,000 leaves generous headroom for the other parameters in these queries and
 * still keeps the round trips few.
 */
export const ID_QUERY_CHUNK = 5_000;

export function chunkIds<T>(ids: readonly T[]): T[][] {
  const out: T[][] = [];
  for (let i = 0; i < ids.length; i += ID_QUERY_CHUNK) {
    out.push(ids.slice(i, i + ID_QUERY_CHUNK));
  }
  return out;
}
...
export async function bulkCreateVersions(
  ...
  for (let attempt = 0; attempt < PARALLEL_WRITE_RETRY_LIMIT && pending.length > 0; attempt++) {
    ...
    const existingVersions = (await Promise.all(chunkIds(cardIds).map((batch) => (
      prisma.contentVersion.findMany({
        where: { targetType: 'card', targetId: { in: batch } },
    ...
    const versionCounts = (await Promise.all(chunkIds(cardIds).map((batch) => (
...
export async function bulkCreateQuestionVersions(
  ...
  for (let attempt = 0; attempt < PARALLEL_WRITE_RETRY_LIMIT && pending.length > 0; attempt++) {
    ...
    const existingVersions = (await Promise.all(chunkIds(questionIds).map((batch) => (
      prisma.contentVersion.findMany({
        where: { targetType: 'question', targetId: { in: batch } },
        select: { targetId: true, contentHash: true },
      })
    )))).flat();
    ...
    const versionCounts = (await Promise.all(chunkIds(questionIds).map((batch) => (
      prisma.contentVersion.groupBy({
        by: ['targetId'],
        where: { targetType: 'question', targetId: { in: batch } },
        _count: true,
