...
export interface BreakModalityRunsOptions<T extends Typed> {
  /**
   * Optional ordering boundary. Modality swaps stay within each contiguous
   * group so a rhythm guard cannot overtake a higher-priority curriculum stage.
   */
  getOrderingGroup?: (item: T) => string | number;
}
 ...
 */
export function breakModalityRuns<T extends Typed>(
  items: T[],
  options: BreakModalityRunsOptions<T> = {},
): T[] {
  if (options.getOrderingGroup && items.length > 0) {
    const result: T[] = [];
    let groupStart = 0;
    while (groupStart < items.length) {
      const group = options.getOrderingGroup(items[groupStart]);
      let groupEnd = groupStart + 1;
      while (
        groupEnd < items.length
        && options.getOrderingGroup(items[groupEnd]) === group
      ) {
        groupEnd += 1;
      }
      result.push(...breakModalityRuns(items.slice(groupStart, groupEnd)));
      groupStart = groupEnd;
    }
    return result;
  }
