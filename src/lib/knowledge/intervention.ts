...
export interface InterventionDecision {
  ...
  variantQuestionRotation?: string;
...
export interface SelectInterventionOptions {
  /** Compute the intervention decision without logging or creating agent jobs. */
...
export async function selectIntervention(
  ...
  options: SelectInterventionOptions = {},
): Promise<InterventionDecision> {
  const effectiveRoll = roll
    ?? deterministicRoll(`${userId}|${cardId}|${dayKey(nowMs)}`);
  ...
  for (const [strat, prob] of Object.entries(INTERVENTION_PROBABILITIES)) {
    ...
    if (effectiveRoll < cumulativeProbability) {
  ...
  switch (strategy) {
    ...
    case 'retreat': {
      ...
      const variant = await findVariantQuestion(userId, cardId, rotation);
      if (variant) {
        decision.variantQuestionId = variant.id;
        decision.variantQuestionRotation = variant.rotation;
      }
      break;
    }
  }
 ...
 */
async function findVariantQuestion(
  userId: string,
  cardId: string,
  sessionRotation: string,
): Promise<{ id: string; rotation: string } | null> {
  const card = await prisma.card.findUnique({
    where: { id: cardId },
  ...
  return question ? { id: question.id, rotation: card.rotation } : null;
}
