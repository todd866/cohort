 ...
 *
 * Detection still fires so the review path can request scaffold alternatives.
 * We do **not** exile hard cards from the pool — difficulty is not a bug.
 */
...
/**
 * Hours to park a leech from selection. Always 0: hard cards stay learnable;
 * scaffolding / alternative stems handle the teaching gap instead of exile.
 */
export function computeLeechSuppressionHours(_suppressionCount: number): number {
  return 0;
}
