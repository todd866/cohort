 ...
 * type alternation, softly spaces recent same-concept items, and expands a
 * running centroid for coverage.
 *
...
export interface WalkOptions {
  ...
  flowWeight?: number;
  /**
   * Signed same-concept proximity weight. The default (-0.35) applies a soft
   * six-position cadence penalty. Set to 0 to disable concept influence, or
   * explicitly pass a positive value to restore the legacy three-position
   * pairing bonus.
   */
...
const CONCEPT_CADENCE_WINDOW = 6;
const LEGACY_CONCEPT_PAIR_WINDOW = 3;
 ...
 *      - Soft penalty for a concept seen in the previous six positions
...
export function orderByManifoldWalk<T extends WalkItem>(
  ...
  while (walk.length < embedded.length) {
    ...
    for (const candidate of embedded) {
      ...
      // -- Concept cadence (avoid immediate same-concept repetition) ---------
      // This is deliberately a score, not an eligibility gate: narrow pools
      // still return every candidate. Positive explicit values retain the old
      // nearby-pairing behavior for callers that intentionally opt into it.
      let conceptProximityScore = 0;
      if (opts.conceptPairWeight !== 0 && candidate.conceptId) {
        const cadenceEnabled = opts.conceptPairWeight < 0;
        const window = cadenceEnabled
          ? CONCEPT_CADENCE_WINDOW
          : LEGACY_CONCEPT_PAIR_WINDOW;
        const lookback = Math.min(window, walk.length);
        for (let k = 1; k <= lookback; k++) {
          const recent = walk[walk.length - k];
          if (recent.conceptId === candidate.conceptId) {
            conceptProximityScore = cadenceEnabled
              ? opts.conceptPairWeight * ((window - k + 1) / window)
              : opts.conceptPairWeight / k;
            break;
          }
        }
      }
 ...
 * items on the same concept appears within 3 positions of each other. This
 * historical metric is retained unchanged for telemetry compatibility; under
 * the default cadence policy, a lower rate can be an intended outcome.
 *
