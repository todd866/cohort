...
export async function requireAuth(): Promise<
  { userId: string; isAdmin?: boolean; imageTier?: string | null; response?: never }
  | { response: NextResponse; userId?: never; isAdmin?: never; imageTier?: never }
> {
  ...
  //
  // imageTier comes from the same callback and is the trusted-reporter anchor
  // (see flags/trusted-reporters.ts). Also optional, and also fails closed.
  return {
    userId: session.user.id,
    isAdmin: session.user.isAdmin === true,
    imageTier: session.user.imageTier ?? null,
  };
}
