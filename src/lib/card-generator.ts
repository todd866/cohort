export type CardComplexity = 1 | 2 | 3 | 4 | 5;
...
export interface GeneratedCard {
  ...
  /** Operative video clip, referenced by its stable slug. Resolved to a
   *  VideoClip id at seed time — MDX cannot know the cuid, which is minted when
   *  the media is cut, and the slug survives a re-cut. */
  clipSlug?: string | null;
  /** 'prompt' = the clip IS the card stem (clip-as-prompt, copyright-tier
   *  only). Null/absent = post-reveal teaching demonstration. */
  clipRole?: string | null;
  /** Says where to look without saying what is there. Shown pre-reveal. */
  clipCaption?: string | null;
  /** Exam views this card serves (e.g. ['gsse','neurosurg']). Undefined when
   *  the source declares none — distinct from [], which would clear existing
   *  assignments on a reseed. */
  moduleNodes?: string[];
  crosslinks?: {
    primary?: string;
    related?: string[];
  ...
  topics: string[];
  /**
   * 1 = trivial recall … 5 = board-style vignette.
   *
   * Widened from 1|2|3 on 2026-09-14. See docs/COMPLEXITY_LADDER.md: tiers 4
   * and 5 are the exam rungs, where the item asks for a mechanism after an
   * unscored diagnostic step and every distractor is eliminable on two
   * independent axes.
   */
  ...
  jurisdiction?: string;
  /** Practice locale for AU/US twins - null=universal, 'au'|'us' */
...
/**
 * Optional file-level curriculum memberships.
 *
 * These are deliberately separate from the descriptive `topic` frontmatter
 * used by content pages. A card may belong to both a broad clinical domain
 * and a Canvas-derived applied-teaching unit, so curriculumTopics are merged
 * with (rather than substituted for) the component/heading topics extracted
 * from MDX.
 */
function parseCurriculumTopics(value: unknown, filePath: string): string[] {
  if (value === undefined) return [];
  if (!Array.isArray(value)) {
    throw new Error(`curriculumTopics must be an array in ${filePath}`);
  }

  const topics = value.map((topic, index) => {
    if (typeof topic !== 'string' || topic.trim().length === 0) {
      throw new Error(`curriculumTopics[${index}] must be a non-empty string in ${filePath}`);
    }
    return topic.trim();
  });
...
export function parseContentFile(
  ...
  const curriculumTopics = parseCurriculumTopics(data.curriculumTopics, filePath);
  ...
  return cards.map((card) => {
    ...
    return {
      ...
      topics: [...new Set([...processed.topics, ...curriculumTopics])],
    };
  });
}
