...
import { releaseOfflinePackStorageForReview } from './offline/pack';

export const OUTBOX_STORAGE_KEY = 'md3:review-queue';
export const OUTBOX_QUARANTINE_KEY = 'md3:outbox-quarantine:v1';
/**
 * A 1,000-item offline pack can produce 2,000 writes when every item is an MCQ
 * (answer plus confidence). Keep another 200 rows for partial refills.
 */
export const REVIEW_OUTBOX_MAX = 2_200;
...
/**
 * The opaque Step 1 route has an intentionally strict four-field contract.
 * The bf6ddebc client briefly persisted generic review observability fields in
 * this body. Strip only those known legacy extras at replay so already-queued
 * answers survive the rolling deploy without weakening the server boundary.
 */
export function replayBodyForOutboxEntry(
  url: string,
  body: Record<string, unknown>,
): Record<string, unknown> {
  if (url !== '/api/usmle/step1/answer') return body;
  return {
    deliveryId: body.deliveryId,
    selectedDisplayLabel: body.selectedDisplayLabel,
...
function writeQueue(entries: StoredOutboxEntry[]): void {
  ...
  try { serialized = JSON.stringify(entries); } catch { return; }
  try {
    localStorage.setItem(OUTBOX_STORAGE_KEY, serialized);
  } catch (error) {
    // Cached study material can be downloaded again; unsent answers cannot.
    // Reclaim only this owner's replaceable pack if browser storage is full.
    const ownerKey = currentOfflineOwnerKey();
    if (
      error instanceof DOMException && error.name === 'QuotaExceededError'
      && ownerKey && entries.some((entry) => entry.ownerKey === ownerKey && entry.kind === 'review')
      && releaseOfflinePackStorageForReview(ownerKey)
    ) {
      try { localStorage.setItem(OUTBOX_STORAGE_KEY, serialized); } catch { /* preserve existing queued writes */ }
    }
  }
  announceChange();
}
...
/** Count-only diagnostics; never expose queued answer bodies to telemetry. */
export function getOutboxStatus(ownerKey: string): { count: number; requiresAuthCount: number } {
  const entries = readQueue().filter((entry) => entry.ownerKey === ownerKey && entry.kind === 'review');
  return {
    count: entries.length,
    requiresAuthCount: entries.filter((entry) => entry.requiresAuth).length,
  };
}
...
// Hundreds of queued answers must not turn a dead radio into hours of sequential
// timeouts. Keep every remaining write for the next online/foreground signal.
    ...
    for (const entry of ownedEntries) {
      ...
      try {
        const res = await fetchWithDeadline(entry.url, {
          ...
          body: JSON.stringify(replayBodyForOutboxEntry(entry.url, entry.body)),
      ...
      // A phone can suspend between any two requests. Persist acknowledgements
      // now instead of leaving an entire successful prefix queued until the end
      // of a multi-minute replay. Merge against current storage so new writes
      // and other-tab clears still win.
      if (generationAtStart === clearGeneration && isOfflineOwnerCurrent(ownerLease)) {
        commitFlushOutcomes(entries, outcomes);
        outcomes.clear();
      }
      if (transportFailures >= MAX_CONSECUTIVE_TRANSPORT_FAILURES) break;
    }
    if (
      generationAtStart === clearGeneration
      && isOfflineOwnerCurrent(ownerLease)
