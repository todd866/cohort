...
describe('checked-in open Step 1 corpus', () => {
  ...
  it('keeps every open item text-only, or image-bearing only with a FOSS media.asset receipt', () => {
    const corpus = loadOpenUsmleQuestionBankFromDisk();
    for (const question of corpus.questions) {
      ...
      const media = question.publicUsmle?.media;
      if (question.imageUrl) {
        expect(media).toEqual(expect.objectContaining({
          kind: 'asset',
          licence: expect.objectContaining({ cls: 'foss' }),
        }));
      } else {
        expect(media?.kind ?? 'none').toBe('none');
      }
    }
  });
