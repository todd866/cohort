...
const ClinicalConditionIdSchema = z.string().trim().min(1).max(160).regex(
...
/** Pedagogical jobs a public Step 1 figure may perform. */
export const PUBLIC_USMLE_FIGURE_JOBS = [
  'pathway',
  'mechanism',
  'anatomy',
  'compare-table',
...
const MediaSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    job: z.enum(PUBLIC_USMLE_FIGURE_JOBS),
    showWhen: z.enum(['after-reveal', 'always']),
    licence: EvidenceLicenceSchema,
    attribution: z.string().trim().min(1),
    sourceUrl: z.string().url().optional(),
    contentHash: z.string().regex(/^sha256:[a-f0-9]{64}$/).optional(),
    /** Required by the Cohort prompt gate and matched exactly to its reviewed asset. */
    clinicalConditionIds: z.array(ClinicalConditionIdSchema).min(1).max(100).optional(),
    clinicalKeyFindings: z.array(z.string().trim().min(1).max(1_000)).min(1).max(100).optional(),
  }).strict(),
]);
 ...
 * `media` (optional; default none) governs whether a figure may be delivered.
...
export const PublicUsmleProvenanceV1Schema = z.object({
  ...
  /** Omitted on text-only items; treated as `{ kind: 'none' }`. */
...
export type PublicUsmleExclusionReason =
  ...
  | 'media-licence-not-public'
  | 'media-asset-mismatch'
  | 'provenance-missing'
  | 'provenance-invalid'
  | 'origin-not-public'
...
export function decidePublicUsmleQuestion(
  ...
  const media = provenance.media ?? { kind: 'none' as const };
  ...
  if (candidate.imageUrl) {
    if (media.kind !== 'asset') {
      return {
        ...
        detail: 'image-bearing questions require publicUsmle.media.kind=asset with a FOSS licence',
      };
    }
    if (!isPublicEvidenceLicence(media.licence)) {
      return {
        eligible: false,
        reason: 'media-licence-not-public',
        detail: `media licence ${media.licence.cls}/${media.licence.id} is not an approved FOSS licence`,
      };
    }
  } else if (media.kind === 'asset') {
    return {
      eligible: false,
      reason: 'media-asset-mismatch',
      detail: `media.asset ${media.assetId} declared but question has no imageUrl`,
    };
  }
