...
const authoredPassage: PublicUsmleProvenanceV1 = {
  schemaVersion: 1,
  origin: 'authored',
  itemText: { licence: 'CC-BY-4.0', attribution: 'md3' },
  evidence: {
    kind: 'passage',
...
describe('decidePublicUsmleQuestion', () => {
  ...
  it('rejects image-bearing items without a media.asset receipt', () => {
  ...
  it('accepts image-bearing items with a FOSS media.asset receipt', () => {
    const provenance: PublicUsmleProvenanceV1 = {
      ...authoredPassage,
      media: {
        kind: 'asset',
        assetId: 'sarcomere-bands-v1',
        job: 'anatomy',
    ...
    const decision = decidePublicUsmleQuestion(candidate({
      imageUrl: '/figures/open/usmle/sarcomere-bands-v1.svg',
    }, provenance));
    expect(decision).toEqual(expect.objectContaining({
      eligible: true,
      reason: 'eligible-authored',
  ...
  it('rejects image-bearing items whose media licence is not FOSS', () => {
    const provenance: PublicUsmleProvenanceV1 = {
      ...authoredPassage,
      media: {
        kind: 'asset',
  ...
  it('rejects a media.asset receipt when no imageUrl is delivered', () => {
    const provenance: PublicUsmleProvenanceV1 = {
      ...authoredPassage,
      media: {
        kind: 'asset',
    ...
    const decision = decidePublicUsmleQuestion(candidate({ imageUrl: null }, provenance));
    expect(decision).toEqual(expect.objectContaining({
      eligible: false,
  ...
  it('treats omitted media as none for text-only items', () => {
    const decision = decidePublicUsmleQuestion(candidate());
    expect(decision).toEqual(expect.objectContaining({ eligible: true }));
  });
