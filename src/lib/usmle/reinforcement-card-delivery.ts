 ...
 * Reads default to the shared catalog; owner-private delivery requires an
 * explicit repository-issued CardReadScope and remains active-epoch bounded.
 */
export async function resolveDeliverableReinforcementCardIds(
  ...
  cardReadScope: CardReadScope = SHARED_CATALOG_CARD_SCOPE,
): Promise<Set<string>> {
  ...
  const cards = await client.card.findMany({
    where: scopedCardWhere(
      cardReadScope,
      withoutRawPublicUsmleReinforcementCards({ id: { in: uniqueIds } }),
    ),
    select: {
      id: true,
      stableId: true,
...
export async function filterDeliverableReinforcementCardRows<T extends { id: string }>(
  ...
  try {
    const deliverableIds = await resolveDeliverableReinforcementCardIds(
      ...
      options.cardReadScope ?? SHARED_CATALOG_CARD_SCOPE,
    );
