...
const answerSchema = z.object({
  ...
  selectedDisplayLabel: z.string().trim().regex(/^[a-z]$/i)
...
/** Shared transport handler; route modules bind the expected delivery surface. */
export async function answerStep1Request(
  request: NextRequest,
        ...
        tasks: [
          {
            ...
            // Authored difficulty is part of the public release fingerprint.
            // Keep empirical facility/attempt counts current without allowing
            // response telemetry to self-revoke a checked-in public item.
            run: () => updateQuestionAnalytics(background.questionId, {
              calibrateDifficulty: false,
            }),
          },
          {
