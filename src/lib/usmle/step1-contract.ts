...
/** Answer-safe media carried only when it is eligible to appear before grading. */
export interface Step1SessionMedia {
  imageUrl: string;
  /** Diagnosis-neutral description that remains useful before grading. */
  preAnswerAlt: string;
  class: 'diagnostic' | 'diagram';
  showWhen: 'always' | 'after-reveal';
  modality?:
    | 'photo' | 'cxr' | 'ct' | 'mri' | 'ecg' | 'us'
    | 'otoscopy' | 'fundoscopy' | 'derm' | 'histology' | 'other';
  attributionText: string;
  licenseUrl: string;
  /** Exact manifest source, present only when its URI is diagnosis-neutral. */
...
export interface Step1AnswerReveal {
  ...
  /**
   * Reviewed description released only after grading. Null means the current
   * delivery has no manifest-admitted visual description for this surface.
   */
  postAnswerAlt: string | null;
  /** Exact manifest source released only after grading when prompt-unsafe. */
