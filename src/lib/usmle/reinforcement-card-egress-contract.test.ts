...
describe('raw USMLE reinforcement Card egress inventory', () => {
  ...
  it('purges every legacy offline payload and keeps new packs on schema v9+', () => {
    const source = read('src/lib/offline/pack.ts');
    const version = source.match(/OFFLINE_PACK_SCHEMA_VERSION\s*=\s*(\d+)/);
    expect(Number(version?.[1])).toBeGreaterThanOrEqual(9);
    expect(source).toContain('parsedSchemaVersion < OFFLINE_PACK_SCHEMA_VERSION');
