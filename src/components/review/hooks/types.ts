...
export interface ReviewItem {
  ...
  /** Source-authoritative placement. `prompt` means the image is required
   *  question content and must render before the answer, even without sidecar
   *  placement metadata. Null retains legacy sidecar inference. */
  ...
  /** Additional server-reviewed explanatory choices; primary source image stays intact. */
  imageAlternatives?: import('@/lib/figures/types').ResolvedImageAlternative[];
  // Operative video clip (cards and questions)
  /** Resolved, signed clip — null when the rights/tier gate withheld it, in
   *  which case the item must fall back to its text-only form rather than
   *  rendering an empty prompt pane. */
  clip?: import('@/components/review/clip-role').ClipPromptData | null;
  /** `prompt` means the clip IS the stem and must render before the answer. */
