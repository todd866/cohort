...
export interface UseSessionLifecycleReturn {
  ...
  sessionId: string;
  /** Record a canonical answer-write attempt in the active visible interval. */
  recordAnswerIntent: (input?: { isCorrect?: boolean }) => void;
}
...
export function useSessionLifecycle({
  ...
  const answerIntentRef = useRef<{ sessionId: string; count: number; correctCount: number } | null>(null);

  const recordAnswerIntent = useCallback((input?: { isCorrect?: boolean }) => {
    const active = activeSessionRef.current;
    const intent = answerIntentRef.current;
    if (!active || !intent || intent.sessionId !== active.id) return;
    intent.count += 1;
    if (input?.isCorrect === true) intent.correctCount += 1;
  }, []);
  ...
  useEffect(() => {
    ...
    const startVisibleInterval = () => {
      ...
      answerIntentRef.current = { sessionId: id, count: 0, correctCount: 0 };
      setSessionId(id);
    ...
    const sendEndEvent = (endReason: 'visibility_hidden' | 'pagehide' | 'unmount') => {
      const active = activeSessionRef.current;
      if (!active) return;
      const answerIntent = answerIntentRef.current?.sessionId === active.id
        ? answerIntentRef.current
        : { sessionId: active.id, count: 0, correctCount: 0 };
      ...
      const endBody = JSON.stringify({
        ...
        metadata: {
          ...
          answerIntent: {
            version: 1,
            scope: 'visible_interval',
            count: answerIntent.count,
            correctCount: answerIntent.correctCount,
          },
        },
      });
  ...
  return { sessionId, recordAnswerIntent };
}
