...
export function CardText({
  ...
  const parts = useMemo(() => {
    // Split by [___] blanks
    const segments: Array<{ type: 'text' | 'blank'; content: string; index?: number; trailing?: string }> = [];
    let blankIndex = 0;
    ...
    while (remaining.length > 0) {
      ...
      // Punctuation touching the blank travels WITH it.
      //
      // The unrevealed slot has to be `inline-block` for its min-width to apply,
      // and an inline-block is an atomic inline box: the boundary between it and
      // the next text node is a break opportunity even with no whitespace there.
      // So "…is a pelvic [___]." could wrap between the slot and the full stop
      // and leave the "." alone on the next line. The U+00A0 trick above cannot
      // reach this case — it rewrites leading whitespace, and there is none.
      //
      // Only punctuation is taken, never a following word: gluing a word to the
      // slot would build an unbreakable run wider than a phone column.
      const trailingPunctuation = remaining.match(/^[.,;:!?)\]}"'»…]+/)?.[0];
      segments.push({
        type: 'blank',
      ...
      if (trailingPunctuation) remaining = remaining.slice(trailingPunctuation.length);
      afterBlank = true;
      blankIndex++;
    }
  ...
  // Prompt media must stay in place when a multi-word answer wraps. Both
  // states occupy the same grid cell; the inert sizing copy reserves height
  // without changing the visible fixed-width blank or exposing answer text.
  if (reserveRevealSpace && parts.some(part => part.type === 'blank')) {
    return (
      <span className="grid">
        <span className="col-start-1 row-start-1 min-w-0">
          <CardText text={text} answers={answers} revealedCount={revealedCount} />
        </span>
        <span aria-hidden="true" inert className="invisible pointer-events-none select-none col-start-1 row-start-1 min-w-0">
          <CardText text={text} answers={answers} revealedCount={999} />
        </span>
      </span>
    );
  }
  ...
  return (
    <span className="whitespace-pre-wrap">
      {parts.map((part, i) => {
        ...
        // Wrap the slot and any punctuation that touched it in one no-break
        // group, so the browser has no opportunity to separate them. Applied to
        // both states: the revealed span is already `inline` and safe on its
        // own, but keeping one shape means the reveal cannot change the wrap.
        const withTrailing = (slot: React.ReactNode) => (
          <span key={i} data-cloze-group className="whitespace-nowrap">
            {slot}
            {part.trailing ? renderText(part.trailing) : null}
          </span>
        );
        if (isRevealed) {
          return withTrailing(
        ...
        return withTrailing(
          <span
            aria-label="blank"
            data-cloze-blank
            // Paint the underline without adding two pixels to the inline
            // box. A real border raised the line height before reveal and
            // moved every image below the stem when that border disappeared.
            className={`relative inline-block mx-0.5 min-w-[2.75ch] select-none align-baseline text-transparent after:absolute after:inset-x-0 after:bottom-0 after:border-b-2 ${
              isNext
                ? 'after:border-[var(--md-primary)]/80'
                : 'after:border-[var(--md-outline-variant)]'
            }`}
          >
            {'  '}
          </span>
