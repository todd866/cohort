...
import type { ReviewItemType } from '@/lib/review/review-intent';
...
interface SessionEmptyStateProps {
  ...
  /** MCQ-only review. Changes the noun, and the way back out. */
 ...
 */
/**
 * The way out of an empty deck.
 *
 * Reported 2026-09-12: the "session complete" state hid the ability to pick a
 * different rotation. Both terminal branches replaced the feed, and the focus
 * control lives in the feed's chrome — so the only control that could help
 * disappeared exactly when it became the only useful one. On a focused deck
 * with nothing due, Retry re-fetches the same empty deck and cannot succeed,
 * which is worse than offering nothing.
 *
 * Any deck can be legitimately empty: unembedded rotation, exhausted queue, a
 * filter that excludes everything. The escape must not be conditional on the
 * deck being non-empty.
 */
function EmptyStateEscape({
  focusRotation,
  onFocusRotationChange,
}: {
  focusRotation: string | null;
  ...
  if (!focusRotation || !onFocusRotationChange) return null;
  return (
    <button
      type="button"
      onClick={() => onFocusRotationChange(null)}
      className="mt-4 block mx-auto rounded-lg px-4 py-2 text-sm font-medium bg-[var(--md-secondary-container)] text-[var(--md-on-secondary-container)] hover:opacity-90"
    >
      Back to scheduled review
    </button>
  );
...
export function SessionEmptyState({
  ...
  // MCQ-only owns its own exhausted state: every branch below is written in
  // cards, and a learner who asked for questions should not be told they have
  // "reviewed all available cards". A thin rotation runs out of MCQs long
  // before it runs out of content, so the way back to Mixed is the point.
  if (isExhausted && itemType === 'question') {
    return (
      <div className="text-center py-12">
        <p className="text-lg font-medium text-[var(--md-on-surface)] mb-2">
          No more MCQs right now
        </p>
        {stats && stats.total > 0 && (
        ...
        <p className="text-sm text-[var(--md-on-surface-variant)]">
          You&apos;ve answered every question this rotation can serve you today.
          Switch to{' '}
          <button
            onClick={() => onReviewModeChange?.('mixed')}
