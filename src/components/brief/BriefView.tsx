...
function DeadlineRow({ deadline }: { deadline: DeadlineView }) {
  const overdue = deadline.urgency === 'overdue';
  const status = overdue
    ? deadline.lifecycle === 'unverified'
      ? `Verify · ${-deadline.daysUntilDue}d past due`
      : `Overdue · ${-deadline.daysUntilDue}d`
    : deadline.urgency === 'due-soon'
      ? deadline.daysUntilDue === 0
        ? 'Due today'
        : `Due in ${deadline.daysUntilDue}d`
      : null;
  ...
  return (
    <li
      className={`border-b py-2.5 last:border-0 ${
        overdue
          ? 'border-[var(--md-error)] text-[var(--md-error)]'
          : 'border-[var(--md-outline-soft)]'
      }`}
    >
      <div className="flex items-baseline justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--md-on-surface)]">{deadline.title}</p>
          <p className="text-xs text-[var(--md-on-surface-variant)]">{deadline.where}</p>
        </div>
        <span className={`shrink-0 text-xs ${overdue ? 'font-semibold text-[var(--md-error)]' : 'text-[var(--md-on-surface-variant)]'}`}>
          {status ? `${status} · ` : ''}{fmt(deadline.dueDate)}
        </span>
      </div>
      {deadline.note ? (
        <p className="mt-1 text-xs text-[var(--md-on-surface-variant)]">{deadline.note}</p>
      ) : null}
      {deadline.sourceUrl ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-1 inline-block text-xs underline text-[var(--md-primary)]"
        >
          Open source
        </a>
      ) : null}
...
export function BriefView({ brief, day }: { brief: CourseBrief; day: DayView }) {
  ...
  const followUp = day.openWatchlist.filter(
    (w) => w.status === 'partial' || w.status === 'awaiting-reply',
  );
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 pb-24 md:pb-6">
      ...
      {followUp.length > 0 && (
        <Section
          title="Follow up"
          subtitle="Partially delivered items and replies that are still outstanding."
        >
          <ul>
            {followUp.map((w) => (
              <WatchRow key={w.item} w={w} />
        ...
        <Section title="Deadlines">
          <ul>
            {day.upcomingDeadlines.map((d) => (
              <DeadlineRow key={`${d.title}-${d.dueDate}`} deadline={d} />
            ))}
          </ul>
        </Section>
      )}
