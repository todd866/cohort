...
interface FigureClientProps {
  ...
  imageKey?: string;
  /** Optional recovery hook for an expired or failed delivered URL. */
  onImageError?: () => void;
}
...
export function FigureClient({
  ...
  const sensitiveGateProps = {
    consentKey: `figure:${figureId}:${imageKey ?? resolvedSrc}`,
    sensitive: shouldGateClientImageMeta(meta, imageKey ?? resolvedSrc),
  };
  ...
  if (shouldRenderOcclusion) {
    ...
    return (
      <SensitiveMediaGate {...sensitiveGateProps}>
        <ImageOcclusion
          id={figureId}
          title={caption}
          src={src}
