...
export function ExamProtocol({
  ...
  const drillExam: DrillExam = exam;
  const stepsRootRef = useRef<HTMLDivElement>(null);
  const hasExpandableSteps = examHasExpandableSteps(exam);
  ...
  return (
    <div ref={stepsRootRef} className="mx-auto max-w-2xl px-5 py-6 pb-28 md:pb-10">
      {/* Header */}
      ...
      {/* The recitable patter — the primary artifact when authored. */}
      {drillExam.algorithm ? (
        <AlgorithmCard algorithm={drillExam.algorithm} />
      ) : drillExam.checklist ? (
        <ChecklistCard checklist={drillExam.checklist} />
      ) : drillExam.drill && drillExam.drill.length > 0 ? (
        <DrillCard lines={drillExam.drill} />
      ) : null}
      ...
      {/* Assessors quiz mid-drill; pre-reading wants everything open. */}
      {hasExpandableSteps && <ExpandAllControls rootRef={stepsRootRef} />}
      ...
      {/* Decision-point rehearsal over the drill — optional, below the phases. */}
      {exam.scenario && <ScenarioTrainer scenario={exam.scenario} />}

      {(exam.rubricSource || (exam.sourceLinks?.length ?? 0) > 0) && (
        <div className="mt-10 border-t border-[var(--md-outline-soft)] pt-4 text-xs leading-relaxed text-[var(--md-on-surface-variant)]">
          {exam.rubricSource ? <p>Source alignment: {exam.rubricSource}</p> : null}
          {exam.sourceLinks && exam.sourceLinks.length > 0 ? (
            <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-2">
              {exam.sourceLinks.map((source) => (
                <li key={source.href}>
                  <a
                    href={source.href}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex min-h-11 items-center font-semibold text-[var(--md-primary)] underline"
                  >
                    {source.label}
                  </a>
                </li>
