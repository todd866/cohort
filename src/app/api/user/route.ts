...
import { userIdInaccessiblePersonalRotations } from '@/lib/personal-rotation-access';
import {
  getCardStats,
  getRecentActivity,
  getWeeklyStats,
...
const feedProfileExplicitSchema = z.object({
  ...
  aiming: z.string().max(200).optional(),
}).strict();
...
export async function GET() {
  ...
  try {
    ...
    const inaccessiblePersonal = await userIdInaccessiblePersonalRotations(userId);
    const broadStatsScope = inaccessiblePersonal.length === 0
      ? undefined
      : { excludeRotations: inaccessiblePersonal };
    ...
    return NextResponse.json({
      profile: {
        ...
        feedProfileExplicit: {
          ...
          aiming: feedProfileExplicit.aiming as string | undefined,
        },
        pollAnswers,
      },
      currentRotation,
