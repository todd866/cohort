...
const BOOLS = ['swRegistered', 'swControlling', 'outboxAuthBlocked'] as const;
...
export async function POST(request: Request) {
  ...
  for (const k of NUMBERS) {
    // The client uses -1 for Cache API failure; zero means a successful empty
    // read. Keep that distinction intact for the production diagnostic.
    const n = k === 'figuresCached' && src[k] === -1 ? -1 : clampNumber(src[k]);
