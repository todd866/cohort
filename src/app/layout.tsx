...
import { hasGuestUserCookie } from "@/lib/guest-user-cookie";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = isCohort
    ? "USMLE Step 1 preparation with free practice questions, explanations, and targeted review."
...
export default async function RootLayout({
  ...
  // Pure cookie read, no database work. This layout is already dynamic, so
  // answering "could anything need importing?" here costs nothing and lets the
  // client skip a no-op authenticated round trip on every steady-state load.
  const guestClaimPending = await hasGuestUserCookie();
  ...
  return (
    <html lang="en" suppressHydrationWarning className={`${geistSans.variable} ${geistMono.variable}`}>
      ...
      >
        <Providers guestClaimPending={guestClaimPending}>
