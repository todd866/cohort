...
const artifactViews = new Map<string, ArtifactView>();

function artifactView(repoRoot: string): ArtifactView {
  const cached = artifactViews.get(repoRoot);
  ...
  const view = {
    ...
    policy,
    textByPath: new Map(policy.generatedTextFiles.map((file) => [file.path, file.text])),
  };
  artifactViews.set(repoRoot, view);
  return view;
}

function artifactPaths(repoRoot: string): Set<string> {
  return artifactView(repoRoot).paths;
}

function artifactText(repoRoot: string, filePath: string): string {
  const view = artifactView(repoRoot);
  const cached = view.textByPath.get(filePath);
  if (cached !== undefined) return cached;

  const source = fs.readFileSync(path.join(repoRoot, filePath), 'utf8');
  view.textByPath.set(filePath, source);
  return source;
}
...
describe('public navigation and static assets', () => {
  ...
  it('advertises only institution entry points with exported pages', () => {
    ...
    expect(institution).toContain('export function isCohortHostname(hostname: string): boolean');
    expect(institution).not.toMatch(/\/md[12]\b|USyd|KAT\d/);

    const compiledInstitution = ts.transpileModule(institution, {
      compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 },
    }).outputText;
    const publicInstitutionModule = { exports: {} as Record<string, unknown> };
    vm.runInNewContext(compiledInstitution, {
      exports: publicInstitutionModule.exports,
      module: publicInstitutionModule,
    });
    const publicIsCohortHostname = publicInstitutionModule.exports.isCohortHostname as (
      hostname: string,
    ) => boolean;
    expect(publicIsCohortHostname('cohort.md')).toBe(true);
    expect(publicIsCohortHostname('www.cohort.md')).toBe(true);
    expect(publicIsCohortHostname('cohort.md:443')).toBe(true);
    expect(publicIsCohortHostname('localhost')).toBe(false);
    expect(publicIsCohortHostname('cohort.md.evil.test')).toBe(false);
  ...
  it('keeps compatibility routes and optional surfaces honest', () => {
    ...
    expect(offlineFigures).not.toMatch(/\/api\/figures|['"`]\/figures\//);
    expect(offlineFigures).toContain('md3-public-open-media-v1');
    expect(offlineFigures).toContain('export async function readCachedFigure');
    expect(offlineFigures).toContain('export async function ensureFiguresCached');
    const offlineImageKey = artifactText(repoRoot, 'src/lib/offline/image-key.ts');
    expect(offlineImageKey).toContain("value.startsWith('/open-media/')");
    expect(offlineImageKey).not.toContain("'/figures/'");
  ...
  it('contains no outgoing production reference to an omitted route', () => {
    ...
    // Whole-artifact scan: ~2s idle, but the 5s default encodes "a unit test
    // this slow is hung", which is the wrong premise for a boundary walk whose
    // cost tracks repo size. Same treatment as the harvester scan (a9efadd0).
  }, 30_000);
  ...
  it('keeps the reachable page graph on the reviewed Step 1 surface', () => {
    ...
    const allowedTargets = new Set([
      ...
      // interpolated form the static scan reports.
      '/api/gamsat/answer',
      '/api/gamsat/corpus',
  ...
  it('ships truthful operator, alpha-access, and deployment contracts', () => {
    ...
    expect(usmleLayout).toContain('USMLE_PUBLIC_SOURCE_CTA');
    ...
    const usmleLanding = artifactText(repoRoot, 'src/app/usmle/page.tsx');
    expect(usmleLanding).toContain('Start Step 1 study');
    expect(usmleLanding).toContain('USMLE_PUBLIC_SOURCE_URL');
    expect(usmleLanding).not.toContain('Steal this bank');
    const readme = artifactText(repoRoot, 'README.md');
    expect(readme).toContain(
      '/usmle is a public early product: guests and signed-in users can study',
    );
    // Derive the advertised counts from the corpus rather than pinning a
    // literal. A hardcoded number is how the published README came to claim 25
    // questions while the release carried 412 — the contract stayed green
    // because it was asserting the stale figure, not the truth.
    const release = JSON.parse(
      artifactText(repoRoot, 'open-content/usmle/step1/release-v1.json'),
    ) as { questionIds: string[] };
    const baseline = JSON.parse(
      artifactText(repoRoot, 'open-content/usmle/step1/baseline-v1.json'),
    ) as { questionIds: string[] };
    expect(release.questionIds.length).toBeGreaterThan(0);

    // A release manifest is not self-contained if its explicitly reviewed FOSS
    // artifact omits a released question or the prompt-media bytes it names.
    // Keep this derived from the artifact view so adding a release ID without
    // updating the distribution policy fails closed.
    const releasedQuestionFiles = new Map<string, {
      path: string;
      imageUrl?: string;
    }>();
    for (const filePath of [...paths].filter((candidate) => (
      candidate.startsWith('open-content/usmle/step1/questions/')
      && candidate.endsWith('.json')
    ))) {
      const question = JSON.parse(artifactText(repoRoot, filePath)) as {
        id?: unknown;
        imageUrl?: unknown;
      };
      if (typeof question.id !== 'string') continue;
      releasedQuestionFiles.set(question.id, {
        path: filePath,
        ...(typeof question.imageUrl === 'string' ? { imageUrl: question.imageUrl } : {}),
      });
    }
    const omittedReleasedIds = release.questionIds.filter((id) => !releasedQuestionFiles.has(id));
    expect(
      omittedReleasedIds,
      `FOSS artifact omits released questions:\n${omittedReleasedIds.join('\n')}`,
    ).toEqual([]);
    const omittedPromptMedia = release.questionIds.flatMap((id) => {
      const question = releasedQuestionFiles.get(id);
      if (!question?.imageUrl?.startsWith('/figures/usmle/step1/')) return [];
      const mediaPath = `open-content/usmle/step1/media/${path.posix.basename(question.imageUrl)}`;
      return paths.has(mediaPath) ? [] : [`${question.path} -> ${mediaPath}`];
    });
    expect(
      omittedPromptMedia,
      `FOSS artifact omits released prompt media:\n${omittedPromptMedia.join('\n')}`,
    ).toEqual([]);

    expect(readme).toMatch(
      new RegExp(`${release.questionIds.length} original, citation-backed`, 'i'),
    );
    expect(readme).toMatch(new RegExp(`${baseline.questionIds.length} original`, 'i'));
  ...
  it('contains no private storage deployment or harvester defaults', () => {
    const policy = artifactView(repoRoot).policy;
    const paths = artifactPaths(repoRoot);
    ...
    for (const filePath of [...paths].sort()) {
      ...
      // Open Step 1 corpus/media dominate the path set; harvester leakage belongs
      // in app/scripts config, not in redistributable question JSON or SVGs.
      if (filePath.startsWith('open-content/usmle/step1/questions/')) continue;
      if (filePath.startsWith('open-content/usmle/step1/media/')) continue;
