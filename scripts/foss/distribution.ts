...
export interface AllowedBinaryFile {
  ...
  notice: string;
  /** Exact reviewed assets may exceed the global text/source bound. Their
   * SHA-256 remains mandatory; this allowance never applies to another path. */
...
export const PUBLIC_API_ROUTE_PATHS: ReadonlyArray<string> = Object.freeze([
  ...
  // Mints a guest study identity. Public by necessity: it is what an anonymous
  // visitor needs before any study request can succeed. Adds no data surface of
  // its own — it returns { ready, isGuest } and delegates all policy, including
  // the per-IP guest-creation rate limit, to requireAuthOrGuest.
  ...
  'src/app/api/gamsat/answer/route.ts',
  'src/app/api/gamsat/corpus/route.ts',
  'src/app/api/gamsat/passage/[id]/route.ts',
...
export const PROTECTED_PUBLIC_FALLBACKS: ReadonlyArray<Readonly<{
  ...
  },
  {
    ...
    // real registry carries USyd Year-3 KAT domain labels, per-domain question
    // and anchor counts, and receipts naming official assessment reports —
    // another institution's material. Pinning the redacted replacement here
    // means an export that ever reverts to the private bytes fails closed
    // rather than publishing them.
    path: 'src/lib/exam-target/registry-data.ts',
    licence: 'MIT',
  ...
  },
  {
    ...
    // generating shards. Public builds retain the generic seam but pin it to
    // an inert implementation so no private registry enters the export.
    path: 'scripts/content/private-content-map-overlays.ts',
    licence: 'MIT',
    sha256: '59ff78d67c6433b09f1a2554fdd0d369b30e79e259bed25c3fd9b9ce7700ebc5',
  },
  {
  ...
  {
    path: 'src/lib/curriculum/usyd-md3-2026-topic-aliases.ts',
    licence: 'MIT',
    sha256: 'b774e054caf1ea996cb1facdd94e5e7524608ac8bd4c031b6c97b70ade93b91a',
  },
  {
    path: 'src/lib/curriculum/usyd-md3-2026.ts',
    licence: 'MIT',
    sha256: '4c198bd0c1145498a857d3b814f82a57176b969ef4325cb38f1c690a74f5f734',
  },
  {
  ...
  },
  {
    ...
    licence: 'MIT',
    sha256: '89af618fafa28c4d934adcc48cf04e02619915ff75cebb029bfdb64d66bcf044',
  },
  {
    path: 'src/lib/institution.ts',
    licence: 'MIT',
    sha256: 'f7ccf616d8493dd75b9939eff090e741fcf68336ced7ac5f7db110009c9ac3ef',
  },
  {
  ...
  },
  {
    ...
    licence: 'MIT',
    sha256: '2be20c1568279b20640560948321ba81f8d3adf73630383dfac0e9bb33e9294f',
  },
  {
    path: 'src/lib/offline/image-key.ts',
    licence: 'MIT',
    sha256: 'a2a7a678ebf2ccb76ca80d52adb193bcaed1ba98f20ad5ff68e85c63c292f350',
  },
  {
    path: 'src/lib/figures/original-image-alternatives.ts',
    licence: 'MIT',
    sha256: '6d21494dec9e0d11331418cff6bae219bae712cb3874d2d5401f2eac9d27fa0e',
  },
  {
    // The real artifact is the sanitized-but-private USyd Canvas curriculum
    // target (course/module-item ids, topic mappings). The public build ships
    // an empty, schema-valid target so mastery-evidence and coverage code
    // compile and run inertly. Added 2026-08-26 after CI's foss-export
    // typecheck caught the unresolvable import — the local export PASS
    // cannot see a missing generated JSON.
    path: 'src/lib/generated/canvas-curriculum-target.json',
    licence: 'MIT',
    sha256: '89b233114d7e6467a518ea6115d474571c87b69cdb97728ccd13eceeb2bc4f93',
  },
  {
    // Same boundary, same date: the real file is the owner-reviewed USyd
    // item-disposition ledger (47k lines of per-item institutional review
    // state). The parser/type module ships; this empty, schema-valid ledger
    // replaces the data so the clean build compiles and every disposition
    // lookup answers neutrally.
    path: 'src/lib/curriculum/usyd-md3-2026-item-dispositions.json',
    licence: 'MIT',
    sha256: 'baef07e99e32178d0ac2161af97b6b7781c2a0d1c146df4e48b388fa2bdbaaa6',
  },
  {
    // Personal pending email approvals are operator data. The public auth hook
  ...
  },
  {
    ...
    licence: 'MIT',
    // Bumped 2026-08-26: the public stub gained personalDeckSharedAccessTier
  ...
  },
  {
    ...
    licence: 'MIT',
    sha256: 'bf4493392c21293a616df1fdd8e0568d882ee718c3fcd3dcb2784cc425520494',
  },
  {
  ...
  {
    // Private review-topic identifiers never enter the public source tree.
    // The server contract stays import-compatible through this empty registry.
    path: 'src/lib/review/review-topic-registry.server.ts',
    licence: 'MIT',
    sha256: 'b40a406b790819d3ebf744e4c0f051f4993f11a0535055e9b6a6ce811f9fc267',
  },
  {
  ...
  },
  {
    ...
    licence: 'MIT',
    // Bumped 2026-08-17: the public shim gained courseCalendarDay, a pure
    // timezone helper with no calendar in it, because teaching-pace-context
    // imports it and the clean FOSS build could not compile without it.
    sha256: 'baed42c4c800154bdaa35139d9175b7dccb69bb1d3be59ace3df0dbabe642b96',
  },
  {
...
const PUBLIC_PACKAGE_SCRIPT_NAMES = new Set([
  ...
  // Installs the 60 open visual assets into the served static path. 345 of the 556
  // released questions reference one, and `public/figures` is a forbidden
  // prefix, so without this the public product renders broken images.
  'usmle:figures:install',
...
function parseAllowedBinaryFiles(value: unknown): AllowedBinaryFile[] {
  ...
  const result = value.map((entry, index) => {
    ...
    if (expected.some((key) => !(key in entry))
      || keys.some((key) => !expected.includes(key) && key !== 'maximumFileBytes')) {
      throw new Error(`allowedBinaryFiles[${index}] has unknown or missing keys`);
    }
    if (entry.maximumFileBytes !== undefined
      && (!Number.isSafeInteger(entry.maximumFileBytes) || Number(entry.maximumFileBytes) <= 0)) {
      throw new Error(`allowedBinaryFiles[${index}].maximumFileBytes must be a positive safe integer`);
    }
    ...
    return {
      ...
      licence,
      notice,
      ...(entry.maximumFileBytes !== undefined ? { maximumFileBytes: Number(entry.maximumFileBytes) } : {}),
    };
  });
...
function inspectFile(
  ...
  const maximumFileBytes = allowedBinary?.maximumFileBytes ?? policy.maximumFileBytes;
  if (buffer.byteLength > maximumFileBytes) {
    issues.push(issue(
      'file-too-large',
      filePath,
      `file is ${buffer.byteLength} bytes; limit is ${maximumFileBytes}`,
    ));
  }
