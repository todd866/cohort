...
describe('FOSS distribution boundary', () => {
  ...
  it('allows a larger reviewed binary only through its exact per-file cap and hash', () => {
    ...
    const bytes = Buffer.alloc(raw.maximumFileBytes + 1, 7);
    fs.writeFileSync(path.join(root, 'app/original.png'), bytes);
    raw.allowedBinaryFiles.push({
      path: 'app/original.png',
    ...
    // The global bound still protects every other candidate, and a larger or
    // merely different asset cannot inherit the original's exact permission.
    fs.writeFileSync(path.join(root, 'app/unreviewed.txt'), 'a'.repeat(bytes.length));
    expect(auditDistributionBoundary(root, policy).issues).toContainEqual(expect.objectContaining({
      code: 'file-too-large', path: 'app/unreviewed.txt',
    }));
    fs.writeFileSync(path.join(root, 'app/original.png'), Buffer.concat([bytes, Buffer.from([8])]));
    const issues = auditDistributionBoundary(root, policy).issues;
    expect(issues).toContainEqual(expect.objectContaining({ code: 'file-too-large', path: 'app/original.png' }));
    expect(issues).toContainEqual(expect.objectContaining({ code: 'third-party-binary-hash-mismatch', path: 'app/original.png' }));
  });

  it.each([0, -1, 1.5, '3000000', Number.MAX_SAFE_INTEGER + 1])(
    'rejects an invalid per-binary size cap %s', (maximumFileBytes) => {
      const root = fixture();
      const policyPath = path.join(root, 'foss/distribution-policy.json');
      const raw = JSON.parse(fs.readFileSync(policyPath, 'utf8'));
      raw.allowedBinaryFiles.push({ path: 'app/original.png', sha256: 'a'.repeat(64), licence: 'MIT', notice: 'LICENSE', maximumFileBytes });
      fs.writeFileSync(policyPath, JSON.stringify(raw));
      expect(() => loadDistributionPolicy(root)).toThrow(/maximumFileBytes/);
    },
  );
  ...
  it('preserves an exact CC BY-SA classification in the export manifest', () => {
    const root = fixture();
    const policyPath = path.join(root, 'foss/distribution-policy.json');
    const raw = JSON.parse(fs.readFileSync(policyPath, 'utf8')) as {
      licenceOverrides: Array<{ path: string; licence: string }>;
    };
    raw.licenceOverrides.push({
  ...
  it('pins dataset-free generated adapters for embedded teaching-data paths', () => {
    ...
    const issues = auditDistributionBoundary(root, policy, {
      verifyExistingExportManifest: false,
    }).issues;
    expect(issues).toContainEqual(expect.objectContaining({
    ...
    // This assertion deliberately walks the checked-in distribution rather
    // than a toy fixture, so its cost scales with the reviewed artifact.
  }, 30_000);
  ...
  it('keeps the checked-in repository boundary green and structurally excludes unsafe roots', () => {
    ...
    expect(policy.includeFiles).toEqual(expect.arrayContaining([
      ...
      'open-content/usmle/step1/visual-assets-v1.json',
    ]));
    ...
    expect(policy.excludeFiles).toEqual(expect.arrayContaining([
      ...
      'src/lib/curriculum/usyd-md3-2026-topic-aliases.ts',
      'src/lib/curriculum/usyd-md1-2026.ts',
      'src/lib/curriculum/usyd-md2-2026.ts',
      'src/lib/curriculum/usyd-md3-2026.ts',
      ...
      'src/lib/toc-guide-access.server.ts',
    ...
    expect(reviewedPaths).toEqual(expect.arrayContaining([
      'prisma/migrations/20260811120000_anki_import_jobs/migration.sql',
      'prisma/schema/anki-import.prisma',
    ]));
    ...
    const publicQuestionModel = publicContentSchema?.text.match(
      /model Question \{[\s\S]*?\n\}/,
    )?.[0];
    expect(publicQuestionModel).toMatch(/\n\s*imageRole\s+String\?/);
    expect(policy.generatedTextFiles).toContainEqual(expect.objectContaining({
    ...
    const publicInstitutionRotations = policy.generatedTextFiles.find(
      (entry) => entry.path === 'src/lib/institution-rotations.ts',
    );
    expect(publicInstitutionRotations?.text).toContain(
      'export const REACHABLE_ROTATIONS: ReadonlySet<string>',
    );
    expect(publicInstitutionRotations?.text).toContain(
      'reachable: ReadonlySet<string> = REACHABLE_ROTATIONS',
    );
    expect(publicInstitutionRotations?.text).not.toContain('ALL_ROTATIONS');
    // The public rewrite must export every symbol the exported sources import
    // from it, or the clean artifact fails to typecheck. This broke on
    // 2026-08-21: defaultPrimaryRotation was added privately and imported by
    // exported server-bootstrap.ts, but the rewrite never grew it, and only
    // CI's foss-export build caught it.
    expect(publicInstitutionRotations?.text).toContain(
      'export function defaultPrimaryRotation',
    );
    // ...without carrying the private acquisition default into the artifact.
    expect(publicInstitutionRotations?.text).not.toContain('DEFAULT_ONBOARDING_ROTATION');
    expect(publicInstitutionRotations?.text).not.toContain("'cah'");
    for (const generatedPath of [
      ...
      'src/lib/offline/image-key.ts',
      'src/lib/figures/original-image-alternatives.ts',
      'src/app/profile/settings/PersonalDocumentsDestination.tsx',
      'src/lib/rotation-labels.ts',
      'src/lib/rotations.ts',
    ...
    expect(policy.generatedTextFiles).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: 'src/lib/curriculum/usyd-md3-2026.ts' }),
    ...
    expect(Object.keys(publicPackage.scripts)).toHaveLength(35);
    ...
    expect(publicPackage.scripts['usmle:figures:install']).toContain(
      'scripts/foss/install-open-figures.ts',
    );
    ...
    expect(publicOfflineFigures).not.toMatch(/\/api\/figures|['"`]\/figures\//);
    expect(publicOfflineFigures).toContain('md3-public-open-media-v1');
    expect(publicOfflineFigures).toContain('export async function readCachedFigure');
    expect(publicOfflineFigures).toContain('export async function ensureFiguresCached');
    ...
    // Whole-repository boundary walk: ~2s idle, but can exceed 30s when the
    // full suite shares the machine with another worker-heavy repository.
    // Same treatment as the harvester scan (a9efadd0) — assertions unchanged,
    // only the hang-detection budget.
  }, 60_000);
});
