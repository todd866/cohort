...
describe('atomic open-corpus seed', () => {
  it('gives the offline corpus transaction an explicit bounded timeout', async () => {
    ...
    const upsert = vi.fn(async () => ({ upserted: 1 }));
    const current = corpus('current');

    await expect(seedOpenCorpusAtomically(host, current, { upsert }))
      .resolves.toEqual({ upserted: 1, retired: 0 });

    expect(transactionOptions).toHaveBeenCalledExactlyOnceWith({ timeout: 120_000 });
    expect(upsert).toHaveBeenCalledWith(transaction, {
      corpus: current,
