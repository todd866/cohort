...
import {
  ...
  type CleanTreeOptions,
} from '../usmle/clean-tree-guard';
...
export function runPublicUsmleAudit(
  argv: string[],
  options: { silent?: boolean; cleanTreeOptions?: CleanTreeOptions } = {},
  ...
  if (
    ...
    // Refuse if tracked corpus files differ from HEAD — otherwise foreign WIP
    // (batch 9 media injection) gets baked into release fingerprints.
    assertCorpusTreeClean(options.cleanTreeOptions);
