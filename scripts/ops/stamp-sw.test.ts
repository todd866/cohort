...
describe('shell cache is build-stamped', () => {
  it('names SHELL_CACHE with the build placeholder, not a fixed version', () => {
    // A fixed 'md3-shell-v1' survived every deploy. networkThenShell serves that
    // cached shell whenever a navigation fetch times out, so a learner on a slow
    // connection got a months-old shell whose router did not know routes added
    // since — rendering "Page not found" for a live route returning 200.
    const sw = readFileSync(resolve(process.cwd(), 'public/sw.js'), 'utf8');
    expect(sw).toMatch(/const SHELL_CACHE = 'md3-shell-__BUILD_STAMP__';/);
    expect(sw).not.toMatch(/const SHELL_CACHE = 'md3-shell-v\d+';/);
  });

  it('keeps the stamped shell in KEEP_CACHES so activate() prunes the previous build', () => {
    const sw = readFileSync(resolve(process.cwd(), 'public/sw.js'), 'utf8');
