...
import {
  assessImageSensitivity,
  type ImageSidecar,
} from '@/lib/images/types';
import { projectServingSidecar } from '@/lib/images/serving-projection';
import { installOriginalDiagrams } from './install-original-diagrams';
...
interface BuildOpts {
  ...
  outFile: string;
  /**
   * Where to write the slim serving projection (what the session function
   * imports at module scope — see src/lib/images/serving-projection.ts).
   * The full catalog at outFile stays for scripts and audits only.
   */
  servingOutFile?: string;
}
interface BuildStats {
  entries: number;
  sensitivityDefaults: number;
  warnings: string[];
}
...
export function buildIndex(opts: BuildOpts): BuildStats {
  ...
  const invalidSensitivity: string[] = [];
  for (const [key, sidecar] of Object.entries(entries)) {
    const assessment = assessImageSensitivity(sidecar);
    if (assessment.verdict === 'invalid') {
      invalidSensitivity.push(`${key}: ${assessment.issues.join('; ')}`);
    }
  }
  if (invalidSensitivity.length > 0) {
    throw new Error(
      '[images:index] invalid sensitivity metadata:\n'
      + invalidSensitivity.map((issue) => `- ${issue}`).join('\n'),
    );
  }
  ...
  if (opts.servingOutFile) {
    const servingEntries = Object.fromEntries(
      Object.entries(stableEntries).map(([key, sidecar]) => [
        key,
        projectServingSidecar(sidecar),
      ]),
    );
    mkdirSync(dirname(opts.servingOutFile), { recursive: true });
    writeFileSync(opts.servingOutFile, JSON.stringify(servingEntries) + '\n', 'utf-8');
  }

  return {
    entries: Object.keys(stableEntries).length,
    sensitivityDefaults: 0,
    warnings,
  };
}
...
export function buildIndexForMode(
  ...
  if (opts.servingOutFile) {
    mkdirSync(dirname(opts.servingOutFile), { recursive: true });
    writeFileSync(opts.servingOutFile, '{}\n', 'utf-8');
  }
  return { entries: 0, sensitivityDefaults: 0, warnings: [] };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    // Public originals are validated and copied for static delivery in every
    // build, including clean FOSS checkouts and offline development builds.
    installOriginalDiagrams({ repoRoot: root, write: true });
    stats = buildIndexForMode({
      sidecarRoot,
      outFile: join(root, 'src', 'data', 'image-library.generated.json'),
      servingOutFile: join(root, 'src', 'data', 'image-serving.generated.json'),
    }, mode);
