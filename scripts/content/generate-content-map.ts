...
import {
  applyPrivateContentMapOverlays,
  type PrivateContentMapCardInput,
} from './private-content-map-overlays';
import { describeError } from '../ops/describe-error.mjs';
...
// never the full card list). Reachability is still asserted via REACHABLE_ROTATIONS.
//
// `surgical-sciences` is the same shape: 17k textbook-plate occlusion cards,
// whose shard came out at 15.7 MB and took CI's `tsc --noEmit` over its 2 GB
// heap (exit 134, "Ineffective mark-compacts near heap limit") while passing
// locally, where the default heap is larger. Like anking it is an imported
// corpus served through the manifold, so the static map buys it nothing.
// `neurosurg` is the same corpus's neuroanatomy half (Kubie), split out of
// surgical-sciences on 2026-09-16, and is served the same way.
const NON_INLINED_ROTATIONS = ['anking', 'surgical-sciences', 'neurosurg'];
...
async function main() {
  ...
  if (mode.readDatabase) {
    try {
      ...
      try {
        // Direct release transport intentionally has a one-connection pool.
        // Keep these two large reads sequential so the second query cannot
        // exhaust its pool-acquisition timeout while the first is still live.
        ...
        questions = await prisma.question.findMany({
          where: withoutRawPublicUsmleQuestions({
            excluded: false,
            contentState: { notIn: ['raw', 'shelved', 'retired'] },
            rotation: { notIn: EXCLUDED_FROM_MAP },
          }),
          select: {
            id: true,
        ...
        // Validate deployment-private metadata overlays while stable/source
        // identities are still present, before normalized shards omit them.
        // The public distribution replaces this seam with a pinned no-op.
        const privateOverlay = applyPrivateContentMapOverlays(
          cards.filter(
            (card): card is StaticCard & PrivateContentMapCardInput => card.stableId !== null,
          ),
          questions,
        );
        console.log(
          `[generate-content-map] Private metadata overlays validated; updated ${privateOverlay.cardsUpdated} card(s) and ${privateOverlay.questionsUpdated} question(s).`,
        );
      } finally {
  ...
  if (fetchError) {
    // describeError, not fetchError.message: a Neon ETIMEDOUT arrives as an
    // AggregateError whose message is empty, which rendered this as a bare
    // "Error: " and aborted the 2026-08-22 release with no stated cause.
    const errorMessage = `[generate-content-map] Could not connect to database. Error: ${describeError(fetchError)}`;
    ...
    // The read and private metadata-overlay validation share one guarded
    // operation. Either can fail after assigning rows, and the overlay itself
    // validates before mutating. Never serialize those unverified rows through
    // the legacy non-strict generator path.
    cards = [];
    questions = [];
    console.warn(
  ...
  typeLines.push('  questionType: string;');
  typeLines.push('  format: string | null;');
  typeLines.push('  variantGroupId: string | null;');
  typeLines.push('  variantType: string | null;');
  ...
  // Reachability guard (audit 2.5): every content-bearing rotation must be in
  // REACHABLE_ROTATIONS — a scheduled block, an explicit-focus deck, or a
  // standalone product. Fail the build on divergence rather than ship content
  // nothing can serve (year1-kat2/kat3 previously did; gamsat later did too).
  const unreachable = unreachableRotations(rotations);
  if (unreachable.length > 0) {
    throw new Error(
      `[generate-content-map] ${unreachable.length} rotation(s) have content but are not reachable ` +
        `by anything (nobody can ever be served them): ${unreachable.join(', ')}. ` +
        `Add each to REACHABLE_ROTATIONS in src/lib/institution-rotations.ts.`,
    );
  }
